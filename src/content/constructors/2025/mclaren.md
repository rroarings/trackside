---
carImage: "../../../assets/constructors/mclaren/car/car-mclaren-01-copy.png"
carImageAlt: "McLaren 2025 Constructor car"
carImageLarge: "../../../assets/constructors/mclaren/car/car-mclaren-01-copy.png"
carImageLargeAlt: "McLaren 2025 Constructor car"
carModel: "MCL39"
careerStats:
  firstEntry: 1966
  racesEntered: 994
  driverChampionships: 12
  constructorChampionships: 10
  grandPrixWins: 202
  podiumPositions: 554
  careerPoints: 7663.5
  polePositions: 174
  fastestLaps: 182
championshipPosition: 1
constructorName: "McLaren"
constructorPoints: 713
dnf: 3
description: "McLaren team description"
engineSupplier: "Mercedes"
fastestLaps: 11
grandPrixWins: 13
grandPrixPoints: 677
heroImage: ""
podiumPositions: 30
polePositions: 10
pubDate: "July 17, 2025"
racesEntered: 991
season: "2025"
sprintPodiums: 5
sprintPoints: 36
sprintPolePositions: 1
sprintWins: 1
teamDrivers: ["Lando Norris", "Oscar Piastri"]
teamLogo: ""
teamPrincipal: "Andrea Stella"
updatedDate: "October 28, 2025"
---

# McLaren

The historic British team McLaren has returned to competitiveness in recent years, with their young driver pairing showing great promise for the future.

## Team Information

**Founded:** 1966  
**Base:** Woking, United Kingdom  
**Championships:** 8 Constructors’, 12 Drivers’  
**Engine:** Mercedes  

## Current Form

McLaren has dominated the 2025 season, clinching the **Constructors’ Championship** with six races to spare after a remarkable run of 16 wins from 20 races. Lando Norris (7 wins) and Oscar Piastri (9 wins) have shared the spotlight, with Norris taking the Drivers’ lead by 1 point after a commanding pole-to-flag victory in Mexico. The team’s consistency is unmatched—scoring in every race—and their upgraded floor package has made the MCL39 the class of the field. Intra-team tension remains a subplot, but performance remains untouchable as they head to Brazil.
